    ...
    public int getGeneratedOutput(@Nonnull Location l, @Nonnull Config data) {
        ...
        if (inv.toInventory() != null && !inv.toInventory().getViewers().isEmpty()) {
            inv.replaceExistingItem(PROGRESS_SLOT,
                canGenerate ? new SlimefunItemStack("PROGRESS_SLOT",Material.GREEN_STAINED_GLASS_PANE, "&aGenerating",
                    ...
                    : new SlimefunItemStack("PROGRESS_SLOT",Material.ORANGE_STAINED_GLASS_PANE, "&cNot Generating",
                    "", "&7Generator has reached maximum capacity.",
                    "", "&7Stored: &6" + Utils.powerFormatAndFadeDecimals(stored) + " J").item()
            );
        }
